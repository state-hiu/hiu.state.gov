...
L.Control.ListMarkers = L.Control.extend({
    ...
    initialize: function(options) {
        ...
        for (var i in this._layer._layers) {
            ...
            console.log(i);*/
              ...
              //just title without link
              //var popupContent = '<div class="title">'+this._layer._layers[i].feature.properties.title+'</div>';

              //link and also calls pop-up box and google analytics
              var popupContent = '<a onClick="goToProduct(this)" href="#'+ this._layer._layers[i].feature.properties.region_id +',' + this._layer._layers[i].feature.properties.product_id + '" class="title">'+this._layer._layers[i].feature.properties.title+'</a>';
              ...
              var newDate = new Date(this._layer._layers[i].feature.properties.date_published);

              this._layer._layers[i].setIcon(productIcon);
              
              /*if ( newDate > past_60_days) {
                this._layer._layers[i].setIcon(productIconNew);
              } else {
                this._layer._layers[i].setIcon(productIcon);
              }*/
    ...
    _createItem: function(layer) {
        ...
        if (layer.feature.properties.interactive == true) {

            console.log('interactive layer in view');
            console.log(layer);
        }
        ...
        if (layer.feature.properties.interactive == true) {
            ...
            var button_text = '<a href=' + baseurl + layer.feature.properties.url + ' class="btn btn-primary" style="padding: 3px 10px;background-color:#269419;color:#fff" type="button">View</a>'
        } else {
            var button_text = '<button class="btn btn-primary dropdown-toggle" style="padding: 3px 10px;" type="button" data-toggle="dropdown">Download'+
        '<span class="caret"></span></button>'+
        '<ul class="dropdown-menu">'+
          '<li><a class="pdf" onclick="ga(&apos;send&apos;, &apos;event&apos;, { eventCategory: &apos;product&apos;, eventAction: &apos;download - PDF&apos;, eventLabel: &apos;#'+layer.feature.properties.region_id+','+layer.feature.properties.product_id+'&apos;, transport: &apos;beacon&apos; });" href="'+layer.feature.properties.product_pdf+'" target="_blank" >PDF</a></li>'+
          '<li><a class="jpg" onclick="ga(&apos;send&apos;, &apos;event&apos;, { eventCategory: &apos;product&apos;, eventAction: &apos;download - JPG&apos;, eventLabel: &apos;#'+layer.feature.properties.region_id+','+layer.feature.properties.product_id+'&apos;, transport: &apos;beacon&apos; });" href="'+layer.feature.properties.product_jpg+'" target="_blank" >JPG</a></li>'+
        '</ul>'
        }
        ...
        button_text + 
        '<a href="https://twitter.com/intent/tweet?text='+ encodeURIComponent(layer.feature.properties.tweet) +'&via=StateHIU&url=https://hiu.state.gov/%23' + layer.feature.properties.region_id + '%2C' + layer.feature.properties.product_id + '" class="tweet" target="_blank"><img src="https://hiu.state.gov/assets/img/Twitter_Logo_Blue.png" style="width: 40px;margin-left:15px"></a>'+
        '</div></div>');
        ...
        if (layer.feature.properties.region_id == activeLayer) {
            ...
            if (layer.feature.properties.interactive == true) {
                ...
                console.log('interactive layer in view');
                console.log(layer);
                link.innerHTML = titleContent + spanContent + interactiveContent;
            }
