...
function createPopUp(currentFeature) {
    ...
    var content = L.DomUtil.create('div', 'title');
...
L.Control.ListMarkers = L.Control.extend({
    ...
    _createItem: function(layer) {
        ...
        '<ul class="dropdown-menu">'+
          '<li><a class="pdf" onclick="ga(&apos;send&apos;, &apos;event&apos;, { eventCategory: &apos;Outbound Link&apos;, eventAction: &apos;click&apos;, eventLabel: event.target.href, transport: &apos;beacon&apos; });" href="'+layer.feature.properties.product_pdf+'">PDF</a></li>'+
          '<li><a class="jpg" href="'+layer.feature.properties.product_jpg+'">JPG</a></li>'+
        '</ul>'+ '<a href="https://twitter.com/intent/tweet?text='+ encodeURIComponent(layer.feature.properties.tweet) +'&via=StateHIU&url=https://hiu.state.gov/%23' + layer.feature.properties.region_id + '%2C' + layer.feature.properties.product_id + '" class="tweet" target="_blank"><img src="{{ site.baseurl }}/assets/img/Twitter_Logo_Blue.png" style="width: 40px;margin-left:15px"></a>'+
        '</div></div>');
           ...
           if ( JpegLength > 1) {
               ...
               // }
                ...
                for (i = 1; i < JpegLength; i++) { 
                    ...
                    $( "div[title='"+currentFeature+"'] .dropdown-menu" ).append('<li><a class="jpg" href="'+layer.feature.properties[jpegArrayKeys[i]]+'">JPG - page '+(i+1)+' of '+JpegLength+'</a></li>');
...
*/
    ...
    _updateList: function() {
        ...
        //https://developers.google.com/analytics/devguides/collection/analyticsjs/events
          ...
          console.log('handleOutbound Link called');
        ...
        //send Google Analytics information when a product pdf is clicked to download
/*
        $( "a.pdf" ).on( "click", function() {
            console.log('pdf clicked');
            var match = $(this).attr('href').match(/#\S+/);
            console.log('url fragment match made: ');
            console.log(match[0] + '&pdf');
            //ga('send', 'pageview', location.pathname + match[0] + '&pdf');
            ga('send', 'event', {
                eventCategory: 'PDF Product Download',
                eventAction: 'click',
...
*/
        //send Google Analytics information when a product jpg is clicked to download
/*
        $( "a.jpg" ).on( "click", function() {
            console.log('jpg clicked');
            var match = $(this).attr('href').match(/#\S+/);
            console.log('url fragment match made: ');
            console.log(match[0] + '&jpg');
            ga('send', 'pageview', location.pathname + match[0] + '&jpg');
        });
*/
