    ...
    L.Hash = L.Class.extend({
        ...
        startListning: function () {
            ...
            return this.map.on("baselayerchange", function (e) {
                ...
                _this.base = (_ref = e.name).replace.apply(_ref, _this.options.formatBase);
                pstate = _this.formatState();
                if (history.pushState) {
                    if (location.hash !== pstate[2] && !_this.moving) {
                        return history.pushState.apply(history, pstate);
        ...
        parseHash: function (hash) {
            ...
            if (args.length > 2) {
                ...
                zoom = parseFloat(args[zIndex]); //<-- Cas Fix --> Support fractional zoom
        ...
        setBase: function (base) {
            ...
            while (i < len) {
                ...
                if (inputs[i].name === 'leaflet-base-layers' && (_ref = this.options.lc._layers[i].name).replace.apply(_ref, this.options.formatBase) === base) {   //<-- ClaudioCas Mod 12/08/2016 [Compatibility Rc3 Leaflet]
        ...
        getBase: function () {
            ...
            while (i < len) {
                if (inputs[i].name === 'leaflet-base-layers' && inputs[i].checked) {
                    ...
                    this.base = (_ref = this.options.lc._layers[i].name).replace.apply(_ref, this.options.formatBase);  //<-- ClaudioCas Mod 12/08/2016 [Compatibility Rc3 Leaflet]
                    return this.base;
                }
                i++; //<-- ClaudioCas Add 12/08/2016 [bug infinite loop]
            }
            return false;
        },
        remove: function () {
