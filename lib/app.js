...
//var img = img.replace(/jpg/, "jpg/products/resized/large");
//var img = img.replace(/hiu-products/, "/hiu-products/products/resized/large");
var img = img.replace(/products.hiu.state.gov/, "products.hiu.state.gov/products/resized/large");
...
//if button's inner text is view instead of download, then it is an interactive product and the button inside the modal needs to be changed
console.log(dropdown.text());
if (dropdown.text() == 'View') {
  console.log('test');
  console.log(dropdown);
  console.log(dropdown.children(":first"));
  console.log(dropdown.children(":first").attr('href'));
  var interactive_href = dropdown.children(":first").attr('href');
  var button_text = '<a href=' + interactive_href + ' class="btn btn-primary" style="padding: 3px 10px;background-color:#269419;color:#fff" type="button">View interactive</a>'
  $('#download-button').replaceWith(button_text);
}
...
$('#myModal').on('hidden.bs.modal', function () {
  ...
  /*var baseLayer = L.tileLayer('https://api.mapbox.com/styles/v1/tgertin/cjddbv3voblhx2sp53yk0asnw/tiles/256/{z}/{x}/{y}?access_token=pk.eyJ1IjoidGdlcnRpbiIsImEiOiJYTW5sTVhRIn0.X4B5APkxkWVaiSg3KqMCaQ',{*/
  var baseLayer = L.tileLayer('https://api.mapbox.com/styles/v1/hiu/ckzzsvukz000214q5fyy3yhjp/tiles/256/{z}/{x}/{y}?access_token=pk.eyJ1IjoiaGl1IiwiYSI6InJWNGZJSzgifQ.xK1ndT3W8XL9lwVZrT6jvQ',{
    minZoom: 1,
    maxZoom: 5,
...
/*
  ...
  legend.onAdd = function (map) {
      ...
      div.innerHTML = '<div id="new-products-legend-outer-div"><div class="new-products-legend circleBase type-new" id="new-products-legend"></div><div class= "new-products-legend new-product-text" style="padding-left:5px;"> New Products</div></div>';
...
*/
  ...
  onEachFeature = (feature, layer) => {
    ...
    //layer.options.className = `my-cool-pattern`;

    if (layer.feature.properties.RegBureau == 'disputed') {

      console.log('disputed mouse-over');

      //layer2.setStyle({fillColor: "blue", fillOpacity: 0.5});
  ...
  function addTopoData(){ 
    ...
    if (window.topoData != undefined) {
        ...
        $.getJSON('assets/data/bureau_outline_topojson9.json')
          .done(topoFunction);
        // $.getJSON('assets/data/eap_with_aksai_chin.json')
        //   .done(topoFunction);
        // $.getJSON('assets/data/sca_only.json')
        //   .done(topoFunction);
    }
      ...
      if (leftLayer == true) {
        ...
        switch (this.feature.properties.RegBureau) {
           ...
           case "EAP":
              ...
              // add a hook here for aksi chin styling
              topoLayer.eachLayer(function (layer2) {
                //console.log('hi');
                //console.log(layer2.feature.properties)
                if (layer2.feature.properties.RegBureau == 'disputed') {

                  //console.log('disputed mouse-over');

                  layer2.setStyle({fillOpacity: 0.5});

                  // layer2.options.className = `my-cool-pattern`;
        ...
        //console.log('enter layer print this');
        //console.log(this);
        ...
        //topoLayer.eachLayer(function (layer2) {
          ...
          //console.log(layer2.feature.properties)
          //layer2.setStyle({fillColor: "blue", fillOpacity: 1});
          //layer2.setStyle({className: 'url(/assets/data/patterns.svg#O-pattern-1-5)'});
        //})

      //   topoLayer.eachLayer(function(layer){
    
      //     //console.log("set the style of "+layer.feature.properties.name);
      //     var density = 150; 
      
      //     layer.setStyle(getStyle(density));
         
      //   }); 

      //   function getStyle(d){
      //     if (d > 100) {return {fillColor: "#ff0000", fillOpacity: 1}}
  ...
  function leaveLayer(){  
      ...
      topoLayer.eachLayer(function (layer2) {
        ...
        if (layer2.feature.properties.RegBureau == 'disputed') {
          ...
          //console.log('AKSAI_CHIN mouse-over');

          layer2.setStyle({fillOpacity: 0});
        }
          ...
          for (var i in features) {
              ...
              if ( newDate > past_60_days) {
                  ...
                  if($('#'+region_id+' .new-product').length <= 0) {
                      $('#'+region_id).append("<div class='new-product-li'>new</div>");
                  }
